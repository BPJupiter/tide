...
r_ogl_instance_buffer_from_size(u64 size)
{
  ...
  if(size > Kilobytes(64))
  {
    ...
    R_OGL_Flush_Buffer *n = push_array(r_ogl_state->buffer_flush_arena, R_OGL_Flush_Buffer, 1);
    n->id = buffer;
...
#undef X
  ...
  for EachEnumVal(R_OGL_ShaderKind, k)
  {
    ...
    // rjf: bind inputs
    R_OGL_Attribute_Array inputs = r_ogl_shader_kind_input_attributes_table[k];
    ...
    // rjf: bind outputs
    R_OGL_Attribute_Array outputs = r_ogl_shader_kind_output_attributes_table[k];
  ...
  bool32 debug_mode = cmd_line_has_flag(cmdln, str8_lit("opengl_debug"));
#if BUILD_DEBUG
  debug_mode = 1;
#endif
  if(debug_mode)
...
r_tex2d_alloc(R_ResourceKind kind, Vec2s32 size, R_Tex2DFormat format, void *data)
{
  ...
  R_OGL_Format_Info gl_fmt_info = r_ogl_format_info_from_tex2dformat(format);
...
r_fill_tex2d_region(R_Handle texture, Rng2s32 subrect, void *data)
{
  ...
  if(t)
  {
    R_OGL_Format_Info fmt_info = r_ogl_format_info_from_tex2dformat(t->fmt);
...
r_end_frame(void)
{
  for(R_OGL_Flush_Buffer *flush_buffer = r_ogl_state->first_buffer_to_flush; flush_buffer != 0; flush_buffer = flush_buffer->next)
  {
      ...
      case R_PassKind_UI:
      {
          ...
          glBindFramebufferScope(GL_FRAMEBUFFER, w->stage_target.fbo)
        {
          for(R_Batch_Group_2D_Node *group_n = rect_batch_groups->first; group_n != 0; group_n = group_n->next)
          {
            ...
            {
              R_OGL_Attribute_Array inputs = r_ogl_shader_kind_input_attributes_table[R_OGL_ShaderKind_Rect];
              u64 off = 0;
              for EachIndex(idx, inputs.count)
              {
                glEnableVertexAttribArray(inputs.v[idx].index);
                glVertexAttribDivisor(inputs.v[idx].index, 1);
                glVertexAttribPointer(inputs.v[idx].index, inputs.v[idx].count, inputs.v[idx].type, GL_FALSE, sizeof(R_Rect2D_Inst), (void *)(off));
