...
internal void
cfg_id_list_push(Arena *arena, CFG_ID_List *list, CFG_ID id)
{
  CFG_ID_Node *n = push_array(arena, CFG_ID_Node, 1);
  SLLQueuePush(list->first, list->last, n);
  list->count += 1;
  n->v = id;
}

internal CFG_ID_List
cfg_id_list_copy(Arena *arena, CFG_ID_List *src)
{
  CFG_ID_List dst = {0};
  for EachNode(n, CFG_ID_Node, src->first)
  {
...
internal void
cfg_node_ptr_list_push(Arena *arena, CFG_Node_Ptr_List *list, CFG_Node *node)
{
  CFG_Node_Ptr_Node *n = push_array(arena, CFG_Node_Ptr_Node, 1);
...
internal void
cfg_node_ptr_list_push_front(Arena *arena, CFG_Node_Ptr_List *list, CFG_Node *node)
{
  CFG_Node_Ptr_Node *n = push_array(arena, CFG_Node_Ptr_Node, 1);
...
internal CFG_Node_Ptr_Array
cfg_node_ptr_array_from_list(Arena *arena, CFG_Node_Ptr_List *list)
{
  CFG_Node_Ptr_Array array = {0};
  array.count = list->count;
  array.v = push_array_no_zero(arena, CFG_Node *, array.count);
  {
    u64 idx = 0;
    for EachNode(n, CFG_Node_Ptr_Node, list->first)
    {
      array.v[idx] = n->v;
      idx += 1;
    }
...
internal void
cfg_schema_table_insert(Arena *arena, CFG_Schema_Table *table, String8 name, MD_Node *schema)
{
  u64 hash = u64_hash_from_str8(name);
  u64 slot_idx = hash%table->slots_count;
  CFG_Schema_Node *node = 0;
  for(CFG_Schema_Node *n = table->slots[slot_idx]; n != 0; n = n->next)
  {
  ...
  if(node == 0)
  {
    node = push_array(arena, CFG_Schema_Node, 1);
...
internal MD_Node *
cfg_schema_from_name(CFG_Schema_Table *table, String8 name)
{
...
internal MD_Node_Ptr_List
cfg_schemas_from_name(Arena *arena, CFG_Schema_Table *table, String8 name)
{
  MD_Node_Ptr_List result = {0};
  {
    Temp scratch = scratch_begin(&arena, 1);
    String8_List tasks = {0};
    String8_Node seed_task = {0, name};
    str8_list_push_node(&tasks, &seed_task);
    for EachNode(task, String8_Node, tasks.first)
    {
...
cfg_node_from_id(CFG_ID id)
{
  ...
  else
  {
    ...
    for(CFG_Node_Ptr_Node *n = cfg_ctx->id_slots[slot_idx].first; n != 0; n = n->next)
    {
      if(n->v->id == id)
      {
        result = n->v;
...
internal String8
cfg_string_from_tree(Arena *arena, CFG_Schema_Table *schema_table, String8 root_path, CFG_Node *root)
{
  Temp scratch = scratch_begin(&arena, 1);
  String8_List strings = {0};
  {
    ...
    for(CFG_Node *c = root; c != &cfg_nil_node; c = rec.next)
    {
      ...
      for(MD_Node_Ptr_Node *n = schemas.first; n != 0 && c_schema == &md_nil_node; n = n->next)
      {
      ...
      if(c->string.size != 0 || c->first == &cfg_nil_node)
      {
        ...
        String8_List c_name_strings = {0};
        {
          bool32 name_can_be_pushed_standalone = 0;
          {
            Temp temp = temp_begin(scratch.arena);
            MD_Tokenise_Result c_name_tokenise = md_tokenise_from_text(temp.arena, c_serialized_string);
            name_can_be_pushed_standalone = (c_name_tokenise.tokens.count == 1 && c_name_tokenise.tokens.v[0].flags & (MD_TokenFlag_Identifier|
      ...
      {
        bool32 is_simple_children_list = 1;
...
cfg_state_alloc(void)
{
  ...
  state->ctx.id_slots = push_array(arena, CFG_Node_Ptr_Slot, state->ctx.id_slots_count);
...
{
  //- rjf: allocate node
  CFG_String_Chunk_Node *node = 0;
  {
    u64 bucket_num = cfg_string_bucket_num_from_size(string.size);
    if(bucket_num == ArrayCount(cfg_string_bucket_chunk_sizes))
    {
      CFG_String_Chunk_Node *best_node = 0;
      CFG_String_Chunk_Node *best_node_prev = 0;
      u64 best_node_size = max_u64;
      {
        for(CFG_String_Chunk_Node *n = state->free_string_chunks[bucket_num-1], *prev = 0; n != 0; (prev = n, n = n->next))
        {
      ...
      else
      {
        ...
        node = (CFG_String_Chunk_Node *)push_array(state->arena, u8, chunk_size);
      }
    }
    else if(bucket_num != 0)
    {
      ...
      {
        node = (CFG_String_Chunk_Node *)push_array(state->arena, u8, cfg_string_bucket_chunk_sizes[bucket_num-1]);
      }
    }
  }
...
cfg_string_release(CFG_State *state, String8 string)
{
  ...
  if(1 <= bucket_num && bucket_num <= ArrayCount(cfg_string_bucket_chunk_sizes))
  {
    ...
    CFG_String_Chunk_Node *node = (CFG_String_Chunk_Node *)string.str;
...
cfg_node_alloc(CFG_State *state)
{
  ...
  {
    CFG_Node_Ptr_Node *cfg_id_node = state->free_id_node;
    if(cfg_id_node != 0)
    {
    ...
    {
      cfg_id_node = push_array(state->arena, CFG_Node_Ptr_Node, 1);
    }
...
cfg_node_release(CFG_State *state, CFG_Node *node)
{
  ...
  for(CFG_Node_Ptr_Node *n = nodes.first; n != 0; n = n->next)
  {
    CFG_Node *c = n->v;
    ...
    c->id = 0;
    c->string = str8_zero();
    for(CFG_Node_Ptr_Node *n = state->ctx.id_slots[slot_idx].first; n != 0; n = n->next)
    {
      if(n->v == c)
      {
...
internal CFG_Node_Ptr_List
cfg_node_ptr_list_from_string(Arena *arena, CFG_State *state, CFG_Schema_Table *schema_table, String8 root_path, String8 string)
{
  CFG_Node_Ptr_List result = {0};
  Temp scratch = scratch_begin(&arena, 1);
  ...
  for MD_EachNode(tln, root->first)
  {
    ...
    for(MD_Node *src_n = tln; !md_node_is_nil(src_n); src_n = rec.next)
    {
      ...
      {
        MD_Node_Ptr_List schemas = cfg_schemas_from_name(scratch.arena, schema_table, dst_active_parent_n->parent->string);
        for(MD_Node_Ptr_Node *n = schemas.first; n != 0 && schema == &md_nil_node; n = n->next)
        {
