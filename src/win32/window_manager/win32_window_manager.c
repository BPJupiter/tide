...
w32_wm_vkey_from_os_key(WM_Key key)
{
  WPARAM result = 0;
  {
    local_persist bool32 initialized = 0;
...
w32_wm_wnd_proc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  if(good)
  {
    ...
    switch(uMsg)
    {
      ...
      {
        bool32 was_down = (lParam & bit31);
        bool32 is_down  = !(lParam & bit32);
      ...
      case WM_SETCURSOR:
      {
        ...
        bool32 is_fullscreen = !(window_style & WS_OVERLAPPEDWINDOW);
        if(window != 0 && window->custom_border && !is_fullscreen)
        {
          bool32 on_border_x = (mouse.x <= window->custom_border_edge_thickness || window_rect.x1-window->custom_border_edge_thickness <= mouse.x);
          bool32 on_border_y = (mouse.y <= window->custom_border_edge_thickness || window_rect.y1-window->custom_border_edge_thickness <= mouse.y);
          on_border = on_border_x || on_border_y;
        }
        if(!w32_wm_resising && !on_border && contains_2f32(window_rect, mouse))
        {
      ...
      case WM_NCHITTEST:
      {
        ...
        {
          bool32 is_default_handled = 0;
          
          // Let the default procedure handle resising areas
          result = DefWindowProc(hwnd, uMsg, wParam, lParam);
          switch (result)
          {
            case HTNOWHERE:
          ...
          if (!is_default_handled)
          {
            ...
            bool32 is_over_top_resize = pos_client.y >= 0 && pos_client.y < frame_y; // + padding;
            bool32 is_over_title_bar  = pos_client.y >= 0 && pos_client.y < window->custom_border_title_thickness;
            
            //- rjf: check against title bar client areas
            bool32 is_over_title_bar_client_area = 0;
            for(W32_WM_Title_Bar_Client_Area *area = window->first_title_bar_client_area;
                area != 0;
                area = area->next)
            {
...
{
  W32_WM_Monitor_Gather_Bundle *bundle = (W32_WM_Monitor_Gather_Bundle *)bundle_ptr;
  WM_Monitor m = {(u64)monitor};
  W32_WM_Monitor_Gather_Node *n = push_array(bundle->arena, W32_WM_Monitor_Gather_Node, 1);
  n->v = m;
...
{
  bool32 custom_border = !!(flags & WM_WindowFlag_CustomBorder);
  bool32 use_default_position = !!(flags & WM_WindowFlag_UseDefaultPosition);
  Vec2f32 pos = rect.p0;
...
internal void
wm_window_set_fullscreen(WM_Window handle, bool32 fullscreen)
{
  W32_WM_Window *window = w32_wm_window_from_handle(handle);
  DWORD window_style = GetWindowLong(window->hwnd, GWL_STYLE);
  bool32 is_fullscreen_already = wm_window_is_fullscreen(handle);
  if(fullscreen)
  {
...
wm_window_push_custom_title_bar_client_area(WM_Window handle, Rng2f32 rect)
{
  ...
  if(window->custom_border)
  {
    W32_WM_Title_Bar_Client_Area *area = push_array(window->paint_arena, W32_WM_Title_Bar_Client_Area, 1);
    if(area != 0)
    {
      area->rect = rect;
...
internal WM_Ext_Window
wm_focused_external_window(void)
{
  HWND hwnd = GetForegroundWindow();
  WM_Ext_Window result = {(u64)hwnd};
  return result;
}

internal void
wm_focus_external_window(WM_Ext_Window handle)
{
...
wm_get_events(Arena *arena, bool32 wait)
{
...
#undef Win32CursorXList
  ...
  if(valid_cursor && !w32_wm_resising)
  {
