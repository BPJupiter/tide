...
mtx_push_op(C_Key buffer_key, MTX_Op op)
{
  ...
  MTX_Mut_Thread *thread = &mtx_shared->mut_threads[hash%mtx_shared->mut_threads_count];
...
internal void
mtx_enqueue_op(MTX_Mut_Thread *thread, C_Key buffer_key, MTX_Op op)
{
...
internal void
mtx_dequeue_op(Arena *arena, MTX_Mut_Thread *thread, C_Key *buffer_key_out, MTX_Op *op_out)
{
  MutexScope(thread->mutex) for(;;)
  {
    u64 unconsumed_size = thread->ring_write_pos - thread->ring_read_pos;
...
{
  MTX_Mut_Thread *mut_thread = (MTX_Mut_Thread *)p;
