...
internal void
mg_msg_list_push(Arena *arena, MG_Msg_List *msgs, MG_Msg *msg)
{
  MG_Msg_Node *n = push_array(arena, MG_Msg_Node, 1);
...
mg_c_string_literal_from_multiline_string(String8 string)
{
  String8_List strings = {0};
  {
    ...
    {
      bool32 is_newline = (off < string.size && (string.str[off] == '\n' || string.str[off] == '\r'));
      bool32 is_ender = (off >= string.size || is_newline);
      if(is_ender)
      {
...
mg_push_map(Arena *arena, u64 slot_count)
{
  ...
  map.slots = push_array(arena, MG_Map_Slot, map.slots_count);
  return map;
}
...
mg_map_ptr_from_string(MG_Map *map, String8 string)
{
  void *result = 0;
  {
    ...
    for(MG_Map_Node *n = slot->first; n != 0; n = n->next)
    {
...
internal MG_Str_Expr *
mg_push_str_expr(Arena *arena, MG_StrExprOp op, MD_Node *node)
{
  MG_Str_Expr *expr = push_array(arena, MG_Str_Expr, 1);
...
mg_str_expr_parse_from_first_opl__min_prec(Arena *arena, MD_Node *first, MD_Node *opl, s8 min_prec)
{
  MG_Str_Expr_Parse_Result parse = {&mg_str_expr_nil};
  {
    ...
    for(;it != opl && !md_node_is_nil(it);)
    {
      ...
      {
        MG_Str_Expr *op_expr = mg_push_str_expr(arena, found_op, it);
    ...
    //- rjf: parse atom
    {
      MG_Str_Expr *atom = &mg_str_expr_nil;
      ...
      {
        MG_Str_Expr_Parse_Result subparse = mg_str_expr_parse_from_first_opl__min_prec(arena, it->first, &md_nil_node, 0);
        atom = subparse.root;
    ...
    for(;it != opl && !md_node_is_nil(it);)
    {
      ...
      MG_Str_Expr_Parse_Result subparse = mg_str_expr_parse_from_first_opl__min_prec(arena, it, opl, mg_str_expr_op_precedence_table[found_op]+1);
...
{
  MG_Str_Expr_Parse_Result parse = mg_str_expr_parse_from_first_opl__min_prec(arena, first, opl, 0);
  return parse;
}
...
{
  MG_Str_Expr_Parse_Result parse = mg_str_expr_parse_from_first_opl__min_prec(arena, root->first, &md_nil_node, 0);
  return parse;
}
...
internal MG_Node_Array
mg_node_array_make(Arena *arena, u64 count)
{
  MG_Node_Array result = {0};
  result.count = count;
...
{
  MG_Node_Array children = mg_node_array_make(arena, md_child_count_from_node(node));
  u64 idx = 0;
...
internal MG_Node_Array
mg_row_from_index(MG_Node_Grid grid, u64 index)
{
  MG_Node_Array result = {0};
...
internal MG_Node_Array
mg_column_from_index(Arena *arena, MG_Node_Grid grid, u64 index)
{
  MG_Node_Array result = {0};
...
internal MD_Node *
mg_node_from_grid_xy(MG_Node_Grid grid, u64 x, u64 y)
{
...
internal MG_Column_Desc_Array
mg_column_desc_array_make(Arena *arena, u64 count, MG_Column_Desc *descs)
{
  MG_Column_Desc_Array result = {0};
  result.count = count;
  result.v = push_array(arena, MG_Column_Desc, result.count);
...
internal u64
mg_column_index_from_name(MG_Column_Desc_Array descs, String8 name)
{
  u64 result = 0;
...
internal String8
mg_string_from_row_desc_idx(MD_Node *row_parent, MG_Column_Desc_Array descs, u64 idx)
{
  String8 result = {0};
  
  // rjf: grab relevant column description
  MG_Column_Desc *desc = 0;
...
internal s64
mg_eval_table_expand_expr__numeric(MG_Str_Expr *expr, MG_Table_Expand_Info *info)
{
  s64 result = 0;
  ...
  switch(op)
  {
    ...
    {
      Temp scratch = scratch_begin(0, 0);
      String8_List left_strs = {0};
      String8_List right_strs = {0};
      ...
      bool32 match = str8_match(left_str, right_str, 0);
...
internal void
mg_eval_table_expand_expr__string(Arena *arena, MG_Str_Expr *expr, MG_Table_Expand_Info *info, String8_List *out)
{
  MG_StrExprOp op = expr->op;
  ...
  switch(op)
  {
    ...
    {
      // rjf: grab left/right
      MG_Str_Expr *left_expr = expr->left;
      MD_Node *left_node = left_expr->node;
      MG_Str_Expr *right_expr = expr->right;
      ...
      u64 row_idx = 0;
      MG_Node_Grid *grid = 0;
      MG_Column_Desc_Array column_descs = {0};
      {
        for(MG_Table_Expand_Task *task = info->first_expand_task; task != 0; task = task->next)
        {
      ...
      {
        bool32 is_multiline = (str8_find_needle(lookup_string, 0, str8_lit("\n"), 0) < lookup_string.size);
        if(is_multiline)
        {
...
internal void
mg_loop_table_column_expansion(Arena *arena, String8 strexpr, MG_Table_Expand_Info *info, MG_Table_Expand_Task *task, String8_List *out)
{
  Temp scratch = scratch_begin(&arena, 1);
    ...
    else
    {
      ...
      for(u64 char_idx = 0; char_idx <= strexpr.size;)
      {
        ...
        if(strexpr.str[char_idx] == '$')
        {
          ...
          MD_Tokenise_Result expr_tokenise = md_tokenise_from_text(scratch.arena, expr_string);
          MD_Parse_Result expr_base_parse = md_parse_from_text_tokens(scratch.arena, str8_lit(""), expr_string, expr_tokenise.tokens);
          MG_Str_Expr_Parse_Result expr_parse = mg_str_expr_parse_from_root(scratch.arena, expr_base_parse.root->first);
...
mg_string_list_from_table_gen(Arena *arena, MG_Map grid_name_map, MG_Map grid_column_desc_map, String8 fallback, MD_Node *gen)
{
  ...
  {
    // rjf: build task list
    MG_Table_Expand_Task *first_task = 0;
    MG_Table_Expand_Task *last_task = 0;
    for MD_EachNode(tag, strexpr_node->first_tag)
    {
      if(str8_match(tag->string, str8_lit("expand"), 0))
      {
        ...
        MG_Node_Grid *grid = mg_map_ptr_from_string(&grid_name_map, table_name);
        MG_Column_Desc_Array *column_descs = mg_map_ptr_from_string(&grid_column_desc_map, table_name);
        ...
        if(grid != 0)
        {
          MG_Table_Expand_Task *task = push_array(scratch.arena, MG_Table_Expand_Task, 1);
    ...
    {
      MG_Table_Expand_Info info = {first_task, fallback};
      if(first_task != 0)
      {
...
mg_layer_key_from_path(String8 path)
{
  ...
  String8_List path_parts = str8_split_path(scratch.arena, str8_chop_last_slash(str8_skip(path, src_folder_pos+4)));
  String_Join join = {0};
  join.sep = str8_lit("/");
