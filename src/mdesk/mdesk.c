...
internal void
md_msg_list_push(Arena *arena, MD_Msg_List *msgs, MD_Node *node, MD_MsgKind kind, String8 string)
{
...
internal void
md_msg_list_pushf(Arena *arena, MD_Msg_List *msgs, MD_Node *node, MD_MsgKind kind, char *fmt, ...)
{
  va_list args;
  va_start(args, fmt);
...
internal void
md_token_chunk_list_push(Arena *arena, MD_Token_Chunk_List *list, u64 cap, MD_Token token)
{
...
internal MD_Token_Array
md_token_array_from_chunk_list(Arena *arena, MD_Token_Chunk_List *chunks)
{
  MD_Token_Array result = {0};
...
{
  bool32 result = md_node_match(a, b, flags);
  if(result)
  {
...
internal MD_Tokenise_Result
md_tokenise_from_text(Arena *arena, String8 text)
{
  Temp scratch = scratch_begin(&arena, 1);
...
internal MD_Parse_Result
md_parse_from_text_tokens(Arena *arena, String8 filename, String8 text, MD_Token_Array tokens)
{
  Temp scratch = scratch_begin(&arena, 1);
  ...
  MD_Parse_Work_Node broken_work = { 0, MD_ParseWorkKind_Main, root,};
...
{
  Temp scratch = scratch_begin(&arena, 1);
  MD_Tokenise_Result tokenise = md_tokenise_from_text(scratch.arena, text);
  MD_Parse_Result parse = md_parse_from_text_tokens(arena, filename, text, tokenise.tokens); 
  scratch_end(scratch);
  return parse;
}
...
md_string_list_from_tree(Arena *arena, MD_Node *root)
{
  String8_List strings = {0};
  {
    char *indentation = "                                                                                                                                ";
    s32 depth = 0;
    bool32 delimit_with_newlines = 1;
    for(MD_Node *node = root, *next = &md_nil_node; !md_node_is_nil(node); node = next)
    {
      Temp scratch = scratch_begin(&arena, 1);
      ...
      {
        MD_Tokenise_Result tokenise = md_tokenise_from_text(scratch.arena, node_string_sanitized);
        bool32 is_simple_string = (tokenise.tokens.count == 1 && (tokenise.tokens.v[0].flags & (MD_TokenFlag_Identifier|
                                                                                             MD_TokenFlag_Numeric)));
        if(tokenise.tokens.count > 0 && !is_simple_string)
        {
      ...
      bool32 next_delimit_with_newlines = delimit_with_newlines;
...
{
  Temp scratch = scratch_begin(&arena, 1);
  String8_List strings = md_string_list_from_tree(scratch.arena, root);
...
internal void
md_node_ptr_list_push(Arena *arena, MD_Node_Ptr_List *list, MD_Node *node)
{
  MD_Node_Ptr_Node *n = push_array(arena, MD_Node_Ptr_Node, 1);
  n->v = node;
...
internal void
md_node_ptr_list_push_front(Arena *arena, MD_Node_Ptr_List *list, MD_Node *node)
{
  MD_Node_Ptr_Node *n = push_array(arena, MD_Node_Ptr_Node, 1);
  n->v = node;
