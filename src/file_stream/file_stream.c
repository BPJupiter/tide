...
fs_artifact_create(String8 key, bool32 *cancel_signal, AC_Status *status_out, u64 *gen_out)
{
  ProfBeginFunction();
  ...
  bool32 file_is_good = 0;
  File_Properties pre_props = {0};
  if(lane_idx() == 0)
  {
  ...
  bool32 file_handle_is_valid = !file_match(file_zero(), file);
    ...
    for EachInRange(slot_idx, range)
    {
      ...
      for(bool32 write_mode = 0; write_mode <= 1; write_mode += 1)
      {
        ...
        RWMutexScope(stripe->rw_mutex, write_mode)
        {
          for(FS_Node *n = slot->first; n != 0; n = n->next)
          {
            File_Properties props = properties_from_file_path(n->path);
