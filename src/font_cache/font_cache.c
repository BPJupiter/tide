...
fnt_handle_from_tag(FNT_Tag tag)
{
  ...
  FNT_Font_Hash_Node *existing_node = 0;
  {
    for(FNT_Font_Hash_Node *n = fnt_state->font_hash_table[slot_idx].first; n != 0 ; n = n->hash_next)
    {
...
fnt_tag_from_path(String8 path)
{
  ...
  if(existing_node == 0)
  {
    ...
    FNT_Font_Hash_Slot *slot = &fnt_state->font_hash_table[slot_idx];
    existing_node = push_array(fnt_state->permanent_arena, FNT_Font_Hash_Node, 1);
...
fnt_tag_from_static_data_string(String8 *data_ptr)
{
  ...
  //- rjf: allocate & push new node if we don't have an existing one
  FNT_Font_Hash_Node *new_node = 0;
  if(existing_node == 0)
  {
    FNT_Font_Hash_Slot *slot = &fnt_state->font_hash_table[slot_idx];
    new_node = push_array(fnt_state->permanent_arena, FNT_Font_Hash_Node, 1);
    new_node->tag = result;
...
fnt_atlas_region_alloc(Arena *arena, FNT_Atlas *atlas, Vec2s16 needed_size)
{
  ...
  FNT_Atlas_Region_Node *node = 0;
  {
    Vec2s16 n_supported_size = atlas->root_dim;
    for(FNT_Atlas_Region_Node *n = atlas->root, *next = 0; n != 0; n = next, next = 0)
    {
      ...
      bool32 n_can_be_allocated = (n->num_allocated_descendants == 0);
      ...
      FNT_Atlas_Region_Node *best_child = 0;
      if(child_size.x >= needed_size.x && child_size.y >= needed_size.y)
      {
        for(Corner corner = (Corner)0; corner < Corner_COUNT; corner = (Corner)(corner+1))
        {
          ...
          {
            n->children[corner] = push_array(arena, FNT_Atlas_Region_Node, 1);
  ...
  if(node != 0 && node_corner != Corner_Invalid)
  {
    ...
    for(FNT_Atlas_Region_Node *p = node->parent; p != 0; p = p->parent)
    {
      p->num_allocated_descendants += 1;
      FNT_Atlas_Region_Node *parent = p->parent;
      if(parent != 0)
      {
...
fnt_atlas_region_release(FNT_Atlas *atlas, Rng2s16 region)
{
  ...
  Corner node_corner = Corner_Invalid;
  {
    ...
    for(FNT_Atlas_Region_Node *n = atlas->root, *next = 0; n != 0; n = next)
    {
...
internal FNT_Piece *
fnt_piece_chunk_list_push_new(Arena *arena, FNT_Piece_Chunk_List *list, u64 cap)
{
  FNT_Piece_Chunk_Node *node = list->last;
  if(node == 0 || node->count >= node->cap)
  {
    node = push_array(arena, FNT_Piece_Chunk_Node, 1);
...
internal void
fnt_piece_chunk_list_push(Arena *arena, FNT_Piece_Chunk_List *list, u64 cap, FNT_Piece *piece)
{
  FNT_Piece *new_piece = fnt_piece_chunk_list_push_new(arena, list, cap);
  MemoryCopyStruct(new_piece, piece);
}

internal FNT_Piece_Array
fnt_piece_array_from_chunk_list(Arena *arena, FNT_Piece_Chunk_List *list)
{
  FNT_Piece_Array array = {0};
  array.count = list->total_piece_count;
  array.v = push_array_no_zero(arena, FNT_Piece, array.count);
  u64 write_idx = 0;
  for(FNT_Piece_Chunk_Node *node = list->first; node != 0; node = node->next)
  {
...
internal FNT_Piece_Array
fnt_piece_array_copy(Arena *arena, FNT_Piece_Array *src)
{
  FNT_Piece_Array dst = {0};
  dst.count = src->count;
...
internal FNT_Hash_2_Style_Raster_Cache_Node *
fnt_hash2style_from_tag_size_flags(FNT_Tag tag, f32 size, FNT_RasterFlags flags)
{
  ...
  FNT_Hash_2_Style_Raster_Cache_Node *hash2style_node = 0;
  {
    u64 slot_idx = style_hash%fnt_state->hash2style_slots_count;
    FNT_Hash_2_Style_Raster_Cache_Slot *slot = &fnt_state->hash2style_slots[slot_idx];
    for(FNT_Hash_2_Style_Raster_Cache_Node *n = slot->first;
        n != 0;
        n = n->hash_next)
    {
    ...
    if(Unlikely(hash2style_node == 0))
    {
      ...
      hash2style_node = push_array(fnt_state->raster_arena, FNT_Hash_2_Style_Raster_Cache_Node, 1);
      ...
      hash2style_node->utf8_class1_direct_map = push_array_no_zero(fnt_state->raster_arena, FNT_Raster_Cache_Info, 256);
      hash2style_node->hash2info_slots_count = 1024;
      hash2style_node->hash2info_slots = push_array(fnt_state->raster_arena, FNT_Hash_2_Info_Raster_Cache_Slot, hash2style_node->hash2info_slots_count);
    }
  }
...
fnt_run_from_string(FNT_Tag tag, f32 size, f32 base_align_px, f32 tab_size_px, FNT_RasterFlags flags, String8 string)
{
  ...
  FNT_Hash_2_Style_Raster_Cache_Node *hash2style_node = fnt_hash2style_from_tag_size_flags(tag, size, flags);
  ...
  if(hash2style_node->run_slots_frame_index != fnt_state->frame_index)
  {
    ...
    hash2style_node->run_slots = push_array(fnt_state->frame_arena, FNT_Run_Cache_Slot, hash2style_node->run_slots_count);
  ...
  FNT_Run_Cache_Slot *run_slot = &hash2style_node->run_slots[run_slot_idx];
  
  //- rjf: find existing run node for this string
  FNT_Run_Cache_Node *run_node = 0;
  {
    for(FNT_Run_Cache_Node *n = run_slot->first; n != 0; n = n->next)
    {
  ...
  else
  {
    ...
    FNT_Piece_Chunk_List piece_chunks = {0};
    Vec2f32 dim = {0};
    bool32 font_handle_mapped_on_miss = 0;
    ...
    for(u64 idx = 0; idx <= string.size;)
    {
      ...
      bool32 need_another_codepoint = 0;
      if(byte == 0)
      {
        idx += 1;
      }
      else switch(utf8_class[byte>>3])
      {
        ...
        default:
        {
          Unicode_Decode decode = utf8_decode(string.str+idx, string.size-idx);
          idx += decode.inc;
      ...
      bool32 is_tab = (piece_substring.size == 1 && piece_substring.str[0] == '\t');
      if(is_tab)
      {
        run_is_cacheable = 0;
      ...
      u64 piece_hash = 0;
      {
        ...
        if(piece_substring.size > 1)
        {
          ...
          FNT_Hash_2_Info_Raster_Cache_Slot *slot = &hash2style_node->hash2info_slots[slot_idx];
          for(FNT_Hash_2_Info_Raster_Cache_Node *node = slot->first; node != 0; node = node->hash_next)
          {
      ...
      if(info == 0)
      {
        ...
        if(font_handle_mapped_on_miss == 0)
        {
          ...
          FNT_Font_Hash_Node *existing_node = 0;
          {
            for(FNT_Font_Hash_Node *n = fnt_state->font_hash_table[font_slot_idx].first; n != 0 ; n = n->hash_next)
            {
        ...
        if(raster.atlas_dim.x != 0 && raster.atlas_dim.y != 0)
        {
          ...
          for(FNT_Atlas *atlas = fnt_state->first_atlas;; atlas = atlas->next, num_atlases += 1)
          {
            ...
            if(atlas == 0 && num_atlases < 64)
            {
              ...
              atlas->root = push_array(fnt_state->raster_arena, FNT_Atlas_Region_Node, 1);
        ...
        // rjf: allocate & fill & push node
        {
          ...
          else
          {
            u64 slot_idx = piece_hash%hash2style_node->hash2info_slots_count;
            FNT_Hash_2_Info_Raster_Cache_Slot *slot = &hash2style_node->hash2info_slots[slot_idx];
            FNT_Hash_2_Info_Raster_Cache_Node *node = push_array_no_zero(fnt_state->raster_arena, FNT_Hash_2_Info_Raster_Cache_Node, 1);
...
fnt_wrapped_string_lines_from_font_size_string_max(Arena *arena, FNT_Tag font, f32 size, f32 base_align_px, f32 tab_size_px, String8 string, f32 max)
{
  String8_List list = {0};
  {
    ...
    for(FNT_Piece *piece = pieces_first, *next = 0; piece != 0 && piece <= pieces_opl; piece = next)
    {
      ...
      bool32 is_first_byte_of_word = 0;
      bool32 is_first_space_after_word = 0;
      ...
      bool32 is_illegal = (off_px >= max);
      bool32 is_next_illegal = (off_px + advance >= max);
      bool32 is_end = (byte == 0);
...
internal Vec2f32
fnt_dim_from_tag_size_string_list(FNT_Tag tag, f32 size, f32 base_align_px, f32 tab_size_px, String8_List list)
{
  Vec2f32 sum = {0};
...
fnt_init(void)
{
  ...
  fnt_state->font_hash_table = push_array(fnt_state->permanent_arena, FNT_Font_Hash_Slot, fnt_state->font_hash_table_size);
  fnt_reset();
}
...
fnt_reset(void)
{
  ...
  fnt_state->hash2style_slots = push_array(fnt_state->raster_arena, FNT_Hash_2_Style_Raster_Cache_Slot, fnt_state->hash2style_slots_count);
}
