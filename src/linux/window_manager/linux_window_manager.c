...
internal int
lnx_moveresize_code_from_pos(LNX_WM_Window *window, Vec2f32 pos, bool32 *out_is_in_client_area)
{
    bool32 in_non_client_area = 0;
    ...
    if(window->custom_border)
    {
        ...
        if(!contains_2f32(inside_edges_and_title_bar, pos))
        {
            ...
            for EachNode(n, LNX_WM_Window_Client_Area, window->first_client_area)
            {
...
internal WM_Key
lnx_wm_key_from_keysym(KeySym ks, bool32 *out_is_right_sided)
{
    WM_Key key = WM_Key_Null;
    bool32 is_right_sided = 0;
    switch(ks)
    {
        default:
        {
...
wm_init(void)
{
    ...
    // create invisible global window for stuff like clipboard
    {
    ...
    // determine if we have xfixes -> get CLIPBOARD updates
    {
        ...
        // if we have xfixes -> get CLIPBOARD updates
...
wm_window_is_focused(WM_Window handle)
{
  ...
  bool32 result = (w->window == focused_window);
  return result;
}
...
wm_window_clear_custom_border_data(WM_Window handle)
{
  if(wm_window_match(handle, wm_window_zero())) {return;}
  LNX_WM_Window *w = (LNX_WM_Window *)handle.u64[0];
  for(LNX_WM_Window_Client_Area *n = w->first_client_area, *next = 0; n != 0; n = next)
  {
    next = n->next;
...
wm_window_push_custom_title_bar_client_area(WM_Window handle, Rng2f32 rect)
{
  if(wm_window_match(handle, wm_window_zero())) {return;}
  LNX_WM_Window *w = (LNX_WM_Window *)handle.u64[0];
  LNX_WM_Window_Client_Area *n = lnx_wm_state->free_client_area;
  if(n != 0)
  {
  ...
  {
    n = push_array(lnx_wm_state->arena, LNX_WM_Window_Client_Area, 1);
  }
  n->rect = rect;
...
wm_get_events(Arena *arena, bool32 wait)
{
    ...
    for(;;)
    {
        ...
        {
            // no xfixes? -> request new clipboard state
            if(!lnx_wm_state->xfixes_present)
            {
            ...
            bool32 set_mouse_cursor = 0;
            switch(evt.type)
            {
                default:
                {
                    // xfixes clipboard update event
                ...
                // external application wants to paste from us
                ...
                // external application is pasting to us
...
wm_key_is_down(WM_Key key)
{
  bool32 result = 0;
  {
    ...
    bool32 is_down = !!(keys[code/8] & (1u<<(code%8)));
    result = is_down;
  }
  return result;
}
