...
typedef struct UI_Icon_Info UI_Icon_Info;
struct UI_Icon_Info {
    FNT_Tag icon_font;
...
typedef struct UI_Animation_Info UI_Animation_Info;
...
typedef UI_BOX_CUSTOM_DRAW(UI_Box_Custom_Draw_Function_Type);
...
struct UI_Box
{
    ...
    UI_Box_Custom_Draw_Function_Type *custom_draw;
    ...
    Fuzzy_Match_Range_List fuzzy_match_ranges;
...
typedef struct UI_Anim_Params UI_Anim_Params;
struct UI_Anim_Params
{
    f32 initial;
    f32 target;
    f32 rate;
...
typedef struct UI_Anim_Node UI_Anim_Node;
struct UI_Anim_Node
{
    UI_Anim_Node *slot_next;
    UI_Anim_Node *slot_prev;
    UI_Anim_Node *lru_next;
    UI_Anim_Node *lru_prev;
    u64 first_touched_build_index;
    u64 last_touched_build_index;
    UI_Key key;
    UI_Anim_Params params;
    f32 current;
};

typedef struct UI_Anim_Slot UI_Anim_Slot;
struct UI_Anim_Slot
{
    UI_Anim_Node *first;
    UI_Anim_Node *last;
...
struct UI_State
{
    ...
    //- rjf: anim cache
    UI_Anim_Node *free_anim_node;
    UI_Anim_Node *lru_anim_node;
    UI_Anim_Node *mru_anim_node;
    u64 anim_slots_count;
    UI_Anim_Slot *anim_slots;
  
    //- rjf: build state machine state
    bool32 is_in_open_ctx_menu;
    String8 autocomplete_string;
    bool32 tooltip_can_overflow_window;
    UI_Key tooltip_anchor_key;
    String8_Array current_gen_tags;
    ...
    bool32 ctx_menu_touched_this_frame;
    bool32 is_animating;
  
    //- rjf: build parameters
    UI_Icon_Info icon_info;
    UI_Theme *theme;
    UI_Animation_Info animation_info;
    WM_Window window;
    ...
    DR_FStr_List string_hover_fstrs;
    ...
    bool32 ctx_menu_open;
    bool32 next_ctx_menu_open;
    f32 ctx_menu_open_t;
    UI_Key ctx_menu_key;
    bool32 ctx_menu_changed;
...
internal bool32  ui_key_match(UI_Key a, UI_Key b);

////////////////////////////////
//~ rjf: Event Type Functions

internal UI_Event_Node *ui_event_list_push(Arena *arena, UI_Event_List *list, UI_Event *v);
internal void ui_eat_event_node(UI_Event_List *list, UI_Event_Node *node);

////////////////////////////////
//~ rjf: Text Operation Functions

internal bool32 ui_char_is_scan_boundary(u8 c);
internal s64 ui_scanned_column_from_column(String8 string, s64 start_column, Side side);
internal UI_Txt_Op ui_single_line_txt_op_from_event(Arena *arena, UI_Event *event, String8 string, Rng1u64 cursor_range, u64 cursor, u64 mark);
...
internal void ui_scroll_pt_target_idx(UI_Scroll_Pt *v, s64 idx);
internal void ui_scroll_pt_clamp_idx(UI_Scroll_Pt *v, Rng1s64 range);
...
internal bool32 ui_box_is_nil(UI_Box *box);
internal UI_Box_Rec ui_box_rec_df(UI_Box *box, UI_Box *root, u64 sib_member_off, u64 child_member_off);
...
internal bool32    ui_animating_from_state(UI_State *state);
...
//- rjf: event pumping
internal bool32 ui_next_event(UI_Event **ev);
internal void ui_eat_event(UI_Event *ev);

//- rjf: event consumption helpers
internal bool32 ui_key_press(WM_Modifiers mods, WM_Key key);
internal bool32 ui_key_release(WM_Modifiers mods, WM_Key key);
internal bool32 ui_text(u32 character);
internal bool32 ui_slot_press(UI_EventActionSlot slot);
...
internal bool32                ui_string_hover_active(void);
internal DR_FStr_List ui_string_hover_fstrs(Arena *arena);
...
internal void ui_begin_build(WM_Window window, UI_Event_List *events, UI_Icon_Info *icon_info, UI_Theme *theme, UI_Animation_Info *animation_info, f32 real_dt, f32 animation_dt);
...
internal bool32               ui_begin_ctx_menu(UI_Key key);
internal void              ui_end_ctx_menu(void);
internal bool32               ui_ctx_menu_is_open(UI_Key key);
internal bool32               ui_any_ctx_menu_is_open(void);

//- rjf: focus tree coloring
internal bool32               ui_is_focus_hot(void);
internal bool32               ui_is_focus_active(void);

//- rjf: implicit auto-managed tree-based focus state
internal bool32               ui_is_key_auto_focus_active(UI_Key key);
internal bool32               ui_is_key_auto_focus_hot(UI_Key key);
...
internal Vec4f32           ui_color_from_tags_key_extras(UI_Key key, String8_Array extras);
...
internal inline void       ui_box_equip_display_fstrs(UI_Box *box, DR_FStr_List *strings);
internal inline void       ui_box_equip_fuzzy_match_ranges(UI_Box *box, Fuzzy_Match_Range_List *matches);
internal inline void       ui_box_equip_draw_bucket(UI_Box *box, DR_Bucket *bucket);
internal inline void       ui_box_equip_custom_draw(UI_Box *box, UI_Box_Custom_Draw_Function_Type *custom_draw, void *user_data);
...
read_only global UI_Anim_Node ui_nil_anim_node = {
    &ui_nil_anim_node,
    &ui_nil_anim_node,
};

internal f32 ui_anim_(UI_Key key, UI_Anim_Params *params);
#define ui_anim(key, target_val, ...) ui_anim_((key), &(UI_Anim_Params){.target = (target_val), .rate = (ui_state->default_animation_rate), __VA_ARGS__})
