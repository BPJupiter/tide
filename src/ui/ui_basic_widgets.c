...
ui_label_multiline(f32 max, String8 string)
{
  ...
  UI_Box *box = ui_build_box_from_key(0, ui_key_zero());
  String8_List lines = fnt_wrapped_string_lines_from_font_size_string_max(scratch.arena, ui_top_font(), ui_top_font_size(), 0, ui_top_tab_size(), string, max);
  for(String8_Node *n = lines.first; n != 0; n = n->next)
  {
    ui_label(n->string);
  }
  scratch_end(scratch);
...
ui_line_edit(u64 *cursor, u64 *mark, u8 *edit_buffer, u64 edit_buffer_size, u64 *edit_string_size_out, String8 pre_edit_value, String8 string)
{
  ...
  if(is_focus_active)
  {
    ...
    for(UI_Event *evt = 0; ui_next_event(&evt);)
    {
      ...
      UI_Txt_Op op = ui_single_line_txt_op_from_event(scratch.arena, evt, edit_string, r1u64(0, edit_string.size), *cursor, *mark);
...
typedef struct UI_Image_Draw_Data UI_Image_Draw_Data;
...
internal UI_BOX_CUSTOM_DRAW(ui_image_draw)
{
  ...
  {
    R_Rect2D_Inst *inst = dr_rect(box->rect, v4f32(0, 0, 0, 0), 0, 0, 1.f);
    MemoryCopyArray(inst->corner_radii, box->corner_radii);
  }
  else DR_Tex2DSampleKindScope(draw_data->sample_kind)
  {
    R_Rect2D_Inst *inst = dr_img(box->rect, draw_data->region, draw_data->texture, draw_data->tint, 0, 0, 0);
    MemoryCopyArray(inst->corner_radii, box->corner_radii);
  }
  if(draw_data->blur > 0.01f)
  {
    ...
    R_Pass_Params_Blur *blur = dr_blur(intersect_2f32(clip, box->rect), draw_data->blur, 0);
...
internal UI_Signal
ui_expander(bool32 is_expanded, String8 string)
{
...
internal UI_Signal
ui_expanderf(bool32 is_expanded, char *fmt, ...)
{
  Temp scratch = scratch_begin(0, 0);
  va_list args;
  va_start(args, fmt);
...
internal UI_Signal
ui_sort_header(bool32 sorting, bool32 ascending, String8 string)
{
...
internal UI_Signal
ui_sort_headerf(bool32 sorting, bool32 ascending, char *fmt, ...)
{
  Temp scratch = scratch_begin(0, 0);
  va_list args;
  va_start(args, fmt);
...
internal UI_BOX_CUSTOM_DRAW(ui_sat_val_picker_draw)
{
  ...
  {
    R_Rect2D_Inst *inst = dr_rect(pad_2f32(box->rect, -1.f), v4f32(hue_rgb_linear.x, hue_rgb_linear.y, hue_rgb_linear.z, 0), 4.f, 0, 1.f);
    inst->colors[Corner_00] = inst->colors[Corner_01] = v4f32(1, 1, 1, 1);
  }
  
  // rjf: black gradient overlay pt. 1
  {
    R_Rect2D_Inst *inst = dr_rect(pad_2f32(box->rect, -1.f), v4f32(0, 0, 0, 0), 4.f, 0, 1.f);
...
internal UI_BOX_CUSTOM_DRAW(ui_hue_picker_draw)
{
  ...
  for(int seg = 0; seg < 6; seg += 1)
  {
    ...
    R_Rect2D_Inst *inst = dr_rect(rect, v4f32(0, 0, 0, 0), 0, 0, 0.f);
...
internal UI_BOX_CUSTOM_DRAW(ui_alpha_picker_draw)
{
  ...
  // rjf: build gradient
  {
    ...
    R_Rect2D_Inst *inst = dr_rect(rect, v4f32(0, 0, 0, 0), 0, 0, 0);
...
internal void
ui_scroll_list_row_block_chunk_list_push(Arena *arena, UI_Scroll_List_Row_Block_Chunk_List *list, u64 cap, UI_Scroll_List_Row_Block *block)
{
  UI_Scroll_List_Row_Block_Chunk_Node *n = list->last;
  if(n == 0 || n->count >= n->cap)
  {
    n = push_array(arena, UI_Scroll_List_Row_Block_Chunk_Node, 1);
    n->cap = cap;
    n->v = push_array_no_zero(arena, UI_Scroll_List_Row_Block, n->cap);
...
internal UI_Scroll_List_Row_Block_Array
ui_scroll_list_row_block_array_from_chunk_list(Arena *arena, UI_Scroll_List_Row_Block_Chunk_List *list)
{
  UI_Scroll_List_Row_Block_Array array = {0};
  array.count = list->total_count;
  array.v = push_array_no_zero(arena, UI_Scroll_List_Row_Block, array.count);
  u64 idx = 0;
  for(UI_Scroll_List_Row_Block_Chunk_Node *n = list->first; n != 0; n = n->next)
  {
...
internal u64
ui_scroll_list_row_from_item(UI_Scroll_List_Row_Block_Array *blocks, u64 item)
{
  u64 result = 0;
  {
    u64 row_idx = 0;
    ...
    {
      UI_Scroll_List_Row_Block *block = &blocks->v[block_idx];
...
internal u64
ui_scroll_list_item_from_row(UI_Scroll_List_Row_Block_Array *blocks, u64 row)
{
  u64 result = 0;
  {
    u64 row_idx = 0;
...
ui_scroll_bar(Axis2 axis, UI_Size off_axis_size, UI_Scroll_Pt pt, Rng1s64 idx_range, s64 view_num_indices)
{
...
thread_static UI_Scroll_Pt *ui_scroll_list_scroll_pt_ptr = 0;
...
internal void
ui_scroll_list_begin(UI_Scroll_List_Params *params, UI_Scroll_Pt *scroll_pt, Vec2s64 *cursor_out, Vec2s64 *mark_out, Rng1s64 *visible_row_range_out, UI_Scroll_List_Signal *signal_out)
{
